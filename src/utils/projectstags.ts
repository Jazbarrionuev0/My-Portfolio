import { Tag } from "../types/types";

const GAN: Tag[] = [
    { title: 'Python', className: 'devicon-python-plain' },
    { title: 'TensorFlow', className: 'devicon-tensorflow-original' },
    { title: 'Keras', className: ' devicon-keras-plain' },
    { title: 'Numpy', className: 'devicon-numpy-original' },
    { title: 'Matplotlib', className: 'devicon-matplotlib-plain' },
]
const SnakeGame: Tag[] = [
    { title: 'Python', className: 'devicon-python-plain' },
    { title: 'OpenCV', className: 'devicon-opencv-plain' },
    { title: 'Numpy', className: 'devicon-numpy-original' },
    { title: 'Cvzone', className: 'devicon--plain' },
]

const ObjectDetection: Tag[] = [
    { title: 'Python', className: 'devicon-python-plain' },
    { title: 'TensorFlow', className: 'devicon-tensorflow-original' },
    { title: 'OpenCV', className: 'devicon-opencv-plain' },
    { title: 'Scikit-learn', className: 'devicon-vscode-plain' },
    { title: 'Numpy', className: 'devicon-numpy-original' },
    { title: 'Matplotlib', className: 'devicon-matplotlib-plain' },
    { title: 'Mediapipe', className: 'devicon-matplotlib-plain' },
]

const Handwritten: Tag[] = [
    { title: 'Python', className: 'devicon-python-plain' },
    { title: 'TensorFlow', className: 'devicon-tensorflow-original' },
    { title: 'OpenCV', className: 'devicon-opencv-plain' },
    { title: 'Numpy', className: 'devicon-numpy-original' },
    { title: 'Matplotlib', className: 'devicon-matplotlib-plain' },
]
const Whispering: Tag[] = [
    { title: 'React', className: 'devicon-react-original' },
    { title: 'Next.js', className: 'devicon-nextjs-original' },
    { title: 'Node.js', className: 'devicon-nodejs-plain' },
    { title: 'TypeScript', className: 'devicon-typescript-plain' },
    { title: 'Net', className: 'devicon-dot-net-plain' },
    { title: 'C#', className: 'devicon-csharp-plain' },
    { title: 'Tailwind', className: 'devicon-tailwindcss-plain' },
    { title: 'RESTful APIs', className: '' },
    { title: 'PostgreSQL', className: 'devicon-postgresql-plain' },

]
const Neopixel: Tag[] = [
    { title: 'TypeScript', className: 'devicon-typescript-plain' },
    { title: 'React', className: 'devicon-react-original' },
    { title: 'Next.js', className: 'devicon-nextjs-original' },
    { title: 'Node.js', className: 'devicon-nodejs-plain' },
    { title: 'Tailwind', className: 'devicon-tailwindcss-plain' },
    { title: 'RESTful APIs', className: '' },
    { title: 'Prisma', className: 'devicon-prisma-original' },
    { title: 'Nest', className: 'devicon-nestjs-original' },
    { title: 'PostgreSQL', className: 'devicon-postgresql-plain' },
    { title: 'Docker', className: 'devicon-docker-plain' },
    { title: 'Digital Ocean', className: 'devicon-digitalocean-original' },
]
const QR: Tag[] = [
    { title: 'HTML5', className: 'devicon-html5-plain' },
    { title: 'CSS', className: 'devicon-css3-plain' },
]

const Bash: Tag[] = [
    { title: 'Bash', className: 'devicon-bash-plain' },
]
const Data: Tag[] = [
    { title: 'Python', className: 'devicon-python-plain' },
    { title: 'Matplotlib', className: 'devicon-matplotlib-plain' },
    { title: 'csv', className: 'devicon-matplotlib-plain' },
    { title: 'Seaborn', className: 'devicon-matplotlib-plain' },

]
const Algorithm: Tag[] = [
    { title: 'Python', className: 'devicon-python-plain' },

]

export { GAN, SnakeGame, ObjectDetection, Handwritten, Whispering, Neopixel, QR, Bash, Data, Algorithm }